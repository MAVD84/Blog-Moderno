...
if ($_SERVER['REQUEST_METHOD'] === 'POST') { verify_csrf(); if (!member_auth_allowed('login')) { flash('Demasiados intentos. Espera 15 minutos.', 'error'); redirect('/member-login.php'); } record_member_auth('login');
    ...
    else { session_regenerate_id(true); $_SESSION['member_id']=(int)$member['id']; $_SESSION['member_last_activity']=time(); $_SESSION['csrf']=bin2hex(random_bytes(32)); flash(empty($member['email_verified_at'])?'Accediste. Aún debes verificar tu correo desde Mi perfil.':'Bienvenido, '.$member['display_name'].'.'); redirect('/community.php'); }
}
