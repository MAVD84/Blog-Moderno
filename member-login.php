...
if ($_SERVER['REQUEST_METHOD'] === 'POST') { verify_csrf(); if (!member_auth_allowed('login')) { flash('Demasiados intentos. Espera 15 minutos.', 'error'); redirect('/member-login.php'); } record_member_auth('login');
    ...
    else { session_regenerate_id(true); $_SESSION['member_id']=(int)$member['id']; flash(empty($member['email_verified_at'])?'Accediste. Aún debes verificar tu correo.':'Bienvenido, '.$member['display_name'].'.'); redirect('/profile.php'); }
}
