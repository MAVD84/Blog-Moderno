...
{
    $stmt=db()->prepare('SELECT (SELECT COUNT(*) FROM post_views WHERE post_id=?) views,(SELECT COUNT(*) FROM post_shares WHERE post_id=?) shares,(SELECT COUNT(*) FROM comments WHERE post_id=? AND aprobado=1) comments,(SELECT COUNT(*) FROM post_reactions WHERE post_id=? AND reaction=1)+(SELECT COUNT(*) FROM post_staff_reactions WHERE post_id=? AND reaction=1) likes,(SELECT COUNT(*) FROM post_reactions WHERE post_id=? AND reaction=-1)+(SELECT COUNT(*) FROM post_staff_reactions WHERE post_id=? AND reaction=-1) dislikes');
    $stmt->execute([$postId,$postId,$postId,$postId,$postId,$postId,$postId]);$stats=$stmt->fetch()?:[];$reaction=0;
    if(is_logged_in()&&current_user_id()){$stmt=db()->prepare('SELECT reaction FROM post_staff_reactions WHERE post_id=? AND user_id=?');$stmt->execute([$postId,current_user_id()]);$reaction=(int)($stmt->fetchColumn()?:0);}
    elseif($memberId){$stmt=db()->prepare('SELECT reaction FROM post_reactions WHERE post_id=? AND member_id=?');$stmt->execute([$postId,$memberId]);$reaction=(int)($stmt->fetchColumn()?:0);}
...
<a href="/index.php">Inicio</a><a href="/community.php">Comunidad</a><?php if (is_logged_in()): ?><a href="/admin.php">Escribir</a><a href="/comments.php">Comentarios</a><?php if (is_admin()): ?><details class="nav-dropdown"><summary>Administrar</summary><div><a href="/community-moderation.php">Publicaciones</a><a href="/users.php">Usuarios</a><a href="/members.php">Lectores</a><a href="/security.php">Seguridad</a><a href="/settings.php">Configuración</a></div></details><?php endif; ?><a href="/account.php">Mi cuenta</a><form class="inline logout-form" method="post" action="/logout.php"><input type="hidden" name="csrf_token" value="<?= csrf_token() ?>"><button class="link danger">Salir del panel</button></form><?php elseif (is_member_logged_in()): ?><a href="/community-write.php">Publicar</a><a href="/member-posts.php">Mis publicaciones</a><a href="/my-comments.php">Comentarios</a><a class="profile-link" href="/profile.php"><?php if(current_member()['avatar']):?><img class="nav-avatar" src="/uploads/<?=e(current_member()['avatar'])?>" alt=""><?php endif;?>Mi perfil</a><form class="inline logout-form" method="post" action="/member-logout.php"><input type="hidden" name="csrf_token" value="<?= csrf_token() ?>"><button class="link danger">Salir</button></form><?php else: ?><a href="/member-login.php">Acceder</a><a href="/register.php">Registrarse</a><?php endif; ?></div>
