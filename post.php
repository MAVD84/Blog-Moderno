...
$stmt=db()->prepare('SELECT c.id,c.parent_id,c.nombre,c.contenido,c.fecha,c.member_id,c.staff_author_id,COALESCE(m.avatar,u.avatar) avatar,COALESCE(m.profile_slug,u.profile_slug) profile_slug,COALESCE(m.profile_public,u.profile_public) profile_public,COALESCE(cr.likes,0)+COALESCE(sr.likes,0) likes,COALESCE(cr.dislikes,0)+COALESCE(sr.dislikes,0) dislikes,CASE WHEN ? > 0 THEN COALESCE((SELECT reaction FROM comment_staff_reactions WHERE comment_id=c.id AND user_id=?),0) ELSE COALESCE((SELECT reaction FROM comment_reactions WHERE comment_id=c.id AND member_id=?),0) END my_reaction FROM comments c LEFT JOIN members m ON m.id=c.member_id LEFT JOIN users u ON u.id=c.staff_author_id LEFT JOIN (SELECT comment_id,SUM(reaction=1) likes,SUM(reaction=-1) dislikes FROM comment_reactions GROUP BY comment_id) cr ON cr.comment_id=c.id LEFT JOIN (SELECT comment_id,SUM(reaction=1) likes,SUM(reaction=-1) dislikes FROM comment_staff_reactions GROUP BY comment_id) sr ON sr.comment_id=c.id WHERE c.post_id=? AND c.aprobado=1 ORDER BY c.fecha');
$staffReactionId=is_logged_in()?(int)(current_user_id()??0):0;$stmt->execute([$staffReactionId,$staffReactionId,$member?(int)$member['id']:0,$id]);$comments=$stmt->fetchAll();
$commentsByParent = [];
...
?>
<article class="article"><?php if ($post['imagen']): ?><div class="cover-wrap"><img class="cover" src="uploads/<?= e($post['imagen']) ?>" alt="<?= e($post['titulo']) ?>" loading="eager" decoding="async"<?= $imageSize ? ' width="' . (int)$imageSize[0] . '" height="' . (int)$imageSize[1] . '"' : '' ?> style="max-width:100%;height:auto;max-height:72vh;object-fit:contain"></div><?php if($post['image_credit']):?><p class="image-credit">Imagen: <?php if($post['image_credit_url']):?><a href="<?=e($post['image_credit_url'])?>" target="_blank" rel="noopener noreferrer nofollow"><?=e($post['image_credit'])?></a><?php else:?><?=e($post['image_credit'])?><?php endif;?></p><?php endif;?><?php endif; ?>
