...
$renderComments = function (int $parentId = 0, int $depth = 0) use (&$renderComments, $commentsByParent, $canComment, $id): void {
    ...
    <article id="comment-<?=(int)$comment['id']?>" class="comment <?= $depth ? 'comment-reply' : '' ?>" style="--reply-depth:<?= min($depth, 4) ?>"><div class="comment-author"><?php $commentProfile=public_profile_url($comment);if($commentProfile):?><a class="author-profile-link" href="<?=e($commentProfile)?>"><?php endif;?><?php if($comment['avatar']):?><img class="avatar" src="/uploads/<?=e($comment['avatar'])?>" alt=""><?php endif;?><strong><?=e($comment['nombre'])?></strong><?php if($commentProfile):?></a><?php endif;?><?php if(!$comment['member_id']):?><span class="author-badge">Autor</span><?php endif;?><small><?=e(format_date($comment['fecha'],true))?></small></div><p><?=nl2br(e($comment['contenido']))?></p><div class="comment-tools"><?php if(is_member_verified()):?><form method="post" action="/comment-reaction.php"><input type="hidden" name="csrf_token" value="<?=csrf_token()?>"><input type="hidden" name="comment_id" value="<?=(int)$comment['id']?>"><button class="reaction <?= (int)$comment['my_reaction']===1?'active':'' ?>" name="reaction" value="1">👍 <?=(int)$comment['likes']?></button><button class="reaction <?= (int)$comment['my_reaction']===-1?'active':'' ?>" name="reaction" value="-1">👎 <?=(int)$comment['dislikes']?></button></form><?php else:?><span>👍 <?=(int)$comment['likes']?> · 👎 <?=(int)$comment['dislikes']?></span><?php endif;?></div>
