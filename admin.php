...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    else { try { $contenido = sanitize_html($contenido); if (trim(strip_tags($contenido)) === '') throw new RuntimeException('El contenido no puede estar vacío.'); $imagen = upload_image($_FILES['imagen'] ?? []); $slug = unique_post_slug($titulo); $stmt = db()->prepare('INSERT INTO posts (titulo, slug, contenido, imagen, author_id, author_name) VALUES (?, ?, ?, ?, ?, ?)'); $stmt->execute([$titulo, $slug, $contenido, $imagen, current_user_id(), current_author_name()]); flash('Artículo publicado.'); redirect(post_url(['slug' => $slug])); } catch (RuntimeException $e) { flash($e->getMessage(), 'error'); } }
}
